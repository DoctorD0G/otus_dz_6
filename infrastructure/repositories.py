...
class SqlAlchemyProductRepository(ProductRepository):
    ...
    def get(self, product_id: int) -> Product:
        ...
        return Product(id=product_orm.id, qunatity=product_orm.quntity, name=product_orm.name, price=product_orm.price)

    def list(self) -> List[Product]:
        products_orm = self.session.query(ProductORM).all()
        return [Product(id=p.id, qunatity=p.quntity, name=p.name, price=p.price) for p in products_orm]
...
class SqlAlchemyOrderRepository(OrderRepository):
    ...
    def get(self, order_id: int) -> Order:
        ...
        products = [Product(id=p.id, qunatity=p.quntity, name=p.name, price=p.price) for p in order_orm.products]
        return Order(id=order_orm.id, products=products, name=order_orm.name, price=order_orm.price)
    ...
    def list(self) -> List[Order]:
        ...
        for order_orm in orders_orm:
            products = [Product(id=p.id, qunatity=p.quntity, name=p.name, price=p.price) for p in order_orm.products]
            orders.append(Order(id=order_orm.id, products=products, name=order_orm.name, price=order_orm.price))
        return orders
