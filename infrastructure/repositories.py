...
class SqlAlchemyProductRepository(ProductRepository):
    ...
    def get(self, product_id: int) -> Product:
        ...
        return Product(id=product_orm.id, quantity=product_orm.quantity, name=product_orm.name, price=product_orm.price)

    def list(self) -> List[Product]:
        products_orm = self.session.query(ProductORM).all()
        return [Product(id=p.id, quantity=p.quantity, name=p.name, price=p.price) for p in products_orm]
...
class SqlAlchemyOrderRepository(OrderRepository):
    ...
    def get(self, order_id: int) -> Order:
        ...
        products = [Product(id=p.id, quantity=p.quantity, name=p.name, price=p.price) for p in order_orm.products]
